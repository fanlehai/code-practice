...
/*
 * 根据已经配置好的分区，在RecordWriter中write时，选择不同的redis进行写入；
 * 
 * 
 */
...
public class PartitionPruningOutputDriver {
	...
	public static class RedisLastAccessOutputFormat extends OutputFormat<RedisKey, Text> {
		...
		public static class RedisLastAccessRecordWriter extends RecordWriter<RedisKey, Text> {
			...
			@Override
			public void write(RedisKey key, Text value) throws IOException, InterruptedException {
				...
				j.hset(MONTH_FROM_INT.get(key.getLastAccessMonth()), key.getField().toString(), value.toString());
			}
