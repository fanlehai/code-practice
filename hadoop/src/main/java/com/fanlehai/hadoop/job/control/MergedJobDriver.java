...
/*
 * 作业归并：
 * 使得加载同一份数据的两个不相关作业共享MapReduce管道。
 * 
 * 作业归并最主要的优点是数据只需要加载和解析一次。
 * 
 * 实现思路：
 * 1. 在map方法中，对数据处理调用两个不同方法处理；同时用context写入是在value前面加上表示a,b等区分数据；
 * 2. 在reduce阶段进行解析时，根据value的第一个字节(a，b)区分数据；
 * 3. 然后用MultiOutput写入不同文件中；
 * 
 */
...
public class MergedJobDriver {
	...
	public static class AnonymizeDistinctMergedReducer extends Reducer<TaggedText, Text, Text, NullWritable> {
		...
		@Override
		protected void reduce(TaggedText key, Iterable<Text> values, Context context)
		...
		private void anonymizeReduce(Text key, Iterable<Text> values, Context context)
				throws IOException, InterruptedException {
		...
		private void distinctReduce(Text key, Iterable<Text> values, Context context)
				throws IOException, InterruptedException {
	...
	public static void main(String[] args) throws Exception {
		...
		MultipleOutputs.addNamedOutput(job, MULTIPLE_OUTPUTS_ANONYMIZE, TextOutputFormat.class, Text.class,
				NullWritable.class);
		MultipleOutputs.addNamedOutput(job, MULTIPLE_OUTPUTS_DISTINCT, TextOutputFormat.class, Text.class,
				NullWritable.class);
