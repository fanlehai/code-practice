package com.fanlehai.hadoop.job.control;
...
import com.fanlehai.hadoop.job.control.JobChainingDriver.UserIdBinningMapper;
import com.fanlehai.hadoop.job.control.JobChainingDriver.UserIdCountMapper;
import com.fanlehai.hadoop.job.control.JobChainingDriver.UserIdSumReducer;
import com.fanlehai.hadoop.job.control.ParallelJobs.AverageReputationMapper;
import com.fanlehai.hadoop.job.control.ParallelJobs.AverageReputationReducer;
...
public class JobControlDriver {
	...
	public static Job getCountingJob(Path postInput, Path outputDirIntermediate) throws IOException {
		...
		// Job countingJob = new Job(new Configuration(),
		// "JobChaining-Counting");

		Job countingJob = Job.getInstance(new Configuration(), "JobChaining-Counting");
	...
	public static Configuration getBinningJobConf(Job countingJob, Path jobchainOutdir, Path userInput,
			Path binningOutput) throws IOException, Exception {
		...
		Job binningJob = Job.getInstance(new Configuration(), "JobChaining-Binning");
		...
		MultipleOutputs.addNamedOutput(binningJob, JobChainingDriver.MULTIPLE_OUTPUTS_BELOW_NAME,
				TextOutputFormat.class, Text.class, Text.class);

		MultipleOutputs.addNamedOutput(binningJob, JobChainingDriver.MULTIPLE_OUTPUTS_ABOVE_NAME,
				TextOutputFormat.class, Text.class, Text.class);
		...
		try {
			for (FileStatus status : userFiles) {
				...
				binningJob.addCacheFile(new URI(status.getPath().toString()));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
