package com.fanlehai.hadoop.serialize.json.multiline;
...
import com.fanlehai.hadoop.serialize.json.multiline.JsonLexer.JsonLexerState;

import java.io.InputStreamReader;
import java.nio.charset.StandardCharsets;

/**
 * A simple parser that can support reading JSON objects from a random point in
 * JSON text. It reads from the supplied stream (which is assumed to be
 * positioned at any arbitrary position inside some JSON text) until it find the
 * first JSON begin-object "{". From this point on it will keep reading JSON
 * objects until it finds one containing a member string that the user supplies.
 * <p/>
...
public class PartitionedJsonParser {
	...
	private enum MemberSearchState {
		FOUND_STRING_NAME, SEARCHING, IN_MATCHING_OBJECT
	}
