 ...
 * A small fraction of bad records are tolerated. When deserialization of a
 * record results in an exception or a null object, an a warning is logged. If
 * the rate of errors crosses a threshold (default is 0.0001 or 0.01%) a
 * RuntimeException is thrown. <br>
 * By default, the threshold is checked at the <i>end</i> of reading. The
 * threshold can be set with configuration variable
 * <code>elephantbird.mapred.input.bad.record.threshold</code>. A value of 0
...
public abstract class LzoRecordReader<K, V> extends RecordReader<K, V> {
	...
	/*
	 * Error out only after threshold rate is reached and we have see this many
	 * errors
	 */
	public static final String BAD_RECORD_MIN_COUNT_CONF_KEY = "elephantbird.mapred.input.bad.record.min";
	public static final String BAD_RECORD_CHECK_AT_CLOSE = // long name is ok,
															// not usually
															// explicitly set.
	...
	/**
	 * Tracks number of of errors in input and throws a Runtime exception if the
	 * rate of errors crosses a limit.
	 * <p>
	 *
	...
	static class InputErrorTracker implements Closeable {
		...
		void incErrors(Throwable cause) {
			...
			LOG.warn("Error while reading an input record (" + numErrors + " out of " + numRecords + " so far ): ",
					cause);
		...
		private void checkErrorThreshold(Throwable cause) {
			if (numErrors > 0 && errorThreshold <= 0) { // no errors are
														// tolerated
			...
			if (numErrors >= minErrors && errRate > errorThreshold) {
				LOG.error(
						numErrors + " out of " + numRecords + " crosses configured threshold (" + errorThreshold + ")");
