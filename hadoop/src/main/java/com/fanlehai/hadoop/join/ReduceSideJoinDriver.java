 ...
 * 
 * 测试数据：
 * 评论数据：https://archive.org/download/stackexchange/stackoverflow.com-Comments.7z
 * 用户数据：https://archive.org/download/stackexchange/stackoverflow.com-Users.7z
 * 根据id连接这两个数据，使得每个评论显示评论者的信息
 */
...
public class ReduceSideJoinDriver extends Configured implements Tool {
	...
	public static void main(String[] args) throws Exception {
		...
		int res = ToolRunner.run(conf, new ReduceSideJoinDriver(), args);
		if (res == 0) {
			System.err.println("something bad happened !");
		} else {
			System.out.println("Reduce Side Join is done !");
		}
		System.exit(res);
	}
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length != 4) {
			System.err.println(
					"Usage: ReduceSideJoin <user data> <comment data> <out> [inner|leftouter|rightouter|fullouter|anti]");
			System.exit(1);
		}
