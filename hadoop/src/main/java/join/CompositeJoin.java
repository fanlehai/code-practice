 ...
 * map. The example facilitates the same.
 *
 * To run: bin/hadoop jar build/hadoop-examples.jar join [-r <i>reduces</i>]
 ...
 * [-joinOp &lt;inner|outer|override&gt;] [<i>in-dir</i>]* <i>in-dir</i>
 * <i>out-dir</i>
 */

/*
 * 输入数据（有CompositeJoinPrepare运行产生，10个分区）： join/user
 * 10个分区key是userid，value是username join/comment 10分区key是userid，value是评论内容text
 * 
 * yarn jar compositejoin.jar join.CompositeJoin -inFormat
 * KeyValueTextInputFormat -joinOp inner join/user join/comment join/composite
 * yarn jar compositejoin.jar join.CompositeJoin -joinOp inner join/user
 * join/comment join/composite
 */
...
public class CompositeJoin extends Configured implements Tool {
	...
	public static class MapComposite extends Mapper<Text, TupleWritable, Text, Text> {
		
		protected void map(Text key, TupleWritable value, Context context) throws IOException, InterruptedException {

			String vText =value.get(0).toString() + value.get(1).toString();
			context.write(key, new Text(vText));
		}
	}
