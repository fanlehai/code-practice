...
/*
 * 本实例根据实际数据生成bloom filter，bloom有四个参数需要了解下：
 ...
 * k = round(log(2.0) * m / n);
 * 
 * 所以只需要估算出n和p即可，m和k可以计算出来(http://hur.st/bloomfilter?n=40000000&p=1.0E-6)
 * 从hdfs中读取一个或多个文件，然后开始通过bloom训练生成bloom filter，用于ReduceSideJoin的bloom过滤优化
 * 
 */
...
public class BloomFilterDriver {
	public static void main(String[] args) throws Exception {
		...
		System.out.println(String.format("Trained Bloom filter with %d entries", numElements));
