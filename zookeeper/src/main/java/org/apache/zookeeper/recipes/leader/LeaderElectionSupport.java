...
public class LeaderElectionSupport implements Watcher {
	 ...
	 * new instance is created using the connectString and sessionTime
	 * specified.
	 * </p>
	 * <p>
	...
	private void makeOffer() throws KeeperException, InterruptedException {
		...
		leaderOffer.setNodePath(zooKeeper.create(rootNodeName + "/" + "n_", hostName.getBytes(),
	...
	private void determineElectionStatus() throws KeeperException, InterruptedException {
		 ...
		 * become the leader. If we're not elected the leader, attempt to stat
		 * the offer just less than us. If they exist, watch for their failure,
		 * but if they don't, become the leader.
		 */
	...
	private void becomeReady(LeaderOffer neighborLeaderOffer) throws KeeperException, InterruptedException {
		...
		logger.info("{} not elected leader. Watching node:{}", leaderOffer.getNodePath(),
				neighborLeaderOffer.getNodePath());

		/*
		 * Make sure to pass an explicit Watcher because we could be sharing
		 * this zooKeeper instance with someone else.
		 */
		Stat stat = zooKeeper.exists(neighborLeaderOffer.getNodePath(), this);

		if (stat != null) {
			logger.debug("We're behind {} in line and they're alive. Keeping an eye on them.",
					neighborLeaderOffer.getNodePath());
			state = State.READY;
			 ...
			 */
			logger.info("We were behind {} but it looks like they died. Back to determination.",
					neighborLeaderOffer.getNodePath());
			determineElectionStatus();
		}
	 ...
	 * the time this method returns, state could have changed so do not depend
	 * on this to be strongly consistent. This method has to read all leader
	 * offers from ZooKeeper to deterime who the leader is (i.e. there is no
	 * caching) so consider the performance implications of frequent invocation.
	 * If there are no leader offers this method returns null.
	 * 
	...
	private List<LeaderOffer> toLeaderOffers(List<String> strings) throws KeeperException, InterruptedException {
		...
		/*
		 * Turn each child of rootNodeName into a leader offer. This is a tuple
		 * of the sequence number and the node name.
		 */
		...
		/*
		 * We sort leader offers by sequence number (which may not be zero-based
		 * or contiguous) and keep their paths handy for setting watches.
		 */
	...
	public String toString() {
		return "{ state:" + state + " leaderOffer:" + leaderOffer + " zooKeeper:" + zooKeeper + " hostName:" + hostName
				+ " listeners:" + listeners + " }";
	}
	 ...
	 * parent of all leader offers for this service. Obviously all processes
	 * that wish to contend for leader status need to use the same root node.
	 * Note: We assume this node already exists.
	 * </p>
	 * 
	 * @return a znode path
	 */
	...
	/**
	 * The hostname of this process. Mostly used as a convenience for logging
	 * and to respond to {@link #getLeaderHostName()} requests.
	 */
